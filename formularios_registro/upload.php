...
if(is_uploaded_file($_FILES["file"]["tmp_name"]))
{
	...
	if(strtoupper($p['extension'])=="PJPEG"||strtoupper($p['extension'])=="GIF"||strtoupper($p['extension'])=="JPG"||strtoupper($p['extension'])=="PNG"){
		...
		$ruta="httpdocs\ole\eventos\archivos\Archivos_Reg_Compradores";
			...
			if(@ftp_login($conn_id,$user,$password))
			{
				...
				if(@ftp_chdir($conn_id,$ruta)){
					...
					if(@ftp_put($conn_id,$nombreArchivo,$_FILES["file"]["tmp_name"],FTP_BINARY)){
						echo "archivos/Archivos_Reg_Compradores/".$nombreArchivo;
					}else{
						echo 0;
		...
		*/
		...
		if(move_uploaded_file($_FILES['file']['tmp_name'], "../archivos/Archivos_Reg_Compradores/".$nombreArchivo)){
			// File path
